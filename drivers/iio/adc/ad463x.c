...
static int ad463x_probe(struct spi_device *spi)
{
	...
	if (device_id == ID_AD463X) {
		ret = ad463x_spi_read_reg(st, AD463X_REG_CHIP_GRADE,
					  &device_id);
		if (ret < 0)
			return ret;
		device_id &= 0x07;
		for (i = 0; i < ARRAY_SIZE(ad463x_id_table); i++) {
			if (ad463x_id_table[i].driver_data == device_id) {
				indio_dev->name = ad463x_id_table[i].name;
				break;
			}
		}
	}
